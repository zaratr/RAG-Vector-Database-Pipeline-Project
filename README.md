...
| `RAG_EMBEDDING_PROVIDER` | `local` | `fastembed` | `local`, `fastembed`, or `openai`; `local` uses deterministic hash embeddings (no model download), `fastembed` produces real 768-dimensional CLIP embeddings; selects the embedding regime (see below) |
